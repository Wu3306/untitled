#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/12/29 10:44
# @Author  : Ryu
# @Site    :
# @File    : test.py
# @Software: PyCharm
from datetime import datetime
import pyodbc
import random
import uuid

class ODBC_MS:
    def __init__(self, DRIVER, SERVER, DATABASE, UID, PWD):
        self.DRIVER = DRIVER
        self.SERVER = SERVER
        self.DATABASE = DATABASE
        self.UID = UID
        self.PWD = PWD
        # initialization

    def __GetConnect(self):
        # Connect to the DB
        if not self.DATABASE:
            raise (NameError, "no setting db info")
        self.conn = pyodbc.connect(DRIVER=self.DRIVER, SERVER=self.SERVER, DATABASE=self.DATABASE, UID=self.UID,
                                   PWD=self.PWD, charset="UTF-8")
        # self.conn = pyodbc.connect(DRIVER=self.DRIVER, SERVER=self.SERVER, DATABASE=self.DATABASE, UID=self.UID, PWD=self.PWD)
        cur = self.conn.cursor()
        if not cur:
            raise (NameError, "connected failed!")
        else:
            print('connect success')
        return cur

    def ExecQuery(self, sql):
        cur = self.__GetConnect()  # 建立链接并创建数据库操作指针
        cur.execute(sql)  # 通过指针来执行sql指令
        ret = cur.fetchall()  # 通过指针来获取sql指令响应数据
        cur.close()  # 游标指标关闭
        self.conn.close()  # 关闭数据库连接
        return ret
    def ExecNoQuery(self, sql):
        cur = self.__GetConnect()
        cur.execute(sql)
        self.conn.commit()  # 连接句柄来提交
        cur.close()
        self.conn.close()

if __name__ == '__main__':
    try:
        now = datetime.now()
        nowstr = now.strftime("%Y-%m-%d")
        person = [['巫啸', '058113', '393'], ['张华', '057928', '386'], ['尹志刚', '057710', '387'], ['华婧丽', '057656', '388'],
                  ['吴素青', '057570', '389'], ['张国平', '057554', '390'], ['贡元', '058321', '391'], ['张文彬', '058217', '392'],
                  ['徐志辉', '058160', '394'], ['徐国栋', '000271', '974']]
        ms = ODBC_MS('{SQL SERVER}', r'172.21.153.239', 'yun11131', 'sa', 'Admin123')
        for i in range(0, 10):
            a = random.randrange(20, 39)
            b = random.randrange(0, 59)
            c = random.randrange(1,10)
            time = nowstr+" 11:"+str(a)+":"+str(b)
            time = datetime.strptime(time , '%Y-%m-%d %H:%M:%S')
            time4 = nowstr+" 11:"+str(a+c)+":"+str(b)
            time4 = datetime.strptime(time4, '%Y-%m-%d %H:%M:%S')
            time3 = '11'+':'+str(a)
            time3 = datetime.strptime(time3, '%H:%M')
            sql = '''
                    insert
                    into
                    KQ_Download
                    (
                        Person_ID,
                        Card_No,
                        Brush_Date,
                        Brush_Time,
                        Moc_No,
                        Data_Flag,
                        Brush_DateTime,
                        Is_Falsity,
                        FileName,
                        eventUuid,
                        Create_DateTime
                    ) VALUES( '''
            sql=sql+""+person[i][2]+","
            sql = sql + "'" +person[i][1] +"',"
            sql = sql + "'" + nowstr+"',"
            sql = sql + "'" + time3+"',"
            sql = sql + "'19',"
            sql = sql + "10,"
            sql = sql + "'" +time+"',"
            sql = sql + "0,"
            sql = sql + "'_,_',"
            sql = sql + "'" +str(uuid.uuid1()) +"',"
            sql = sql + "'" +time4 +"'"
            sql=sql+")"

            print(sql)
            ms.ExecNoQuery(sql)
    except Exception as e:
        print(str(e))
        input('Please press enter key to exit ...')